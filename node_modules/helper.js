...
exports.partyCheck=function(res,userid,connection,date){
  connection.query("SELECT * FROM party WHERE uid=? AND start=?",[userid,date],function(err,results,objects){
    // console.log("partyCheck function(): user id is:"+userid+"and date is:"+date);
    if(err)
    {
...
exports.sendFriendList=function(chalk,connection,res,friendList)
{
  console.log(chalk.blue("helper function:inside sendFriendList"));
  var len=Object.keys(friendList).length;
  var list=[];
  var name,email,id;
  for(var key in friendList)
  {
    if(key!="elements")
    {
      ...
      email="email"+i.toString();
      id="id"+i.toString();
      connection.query("SELECT * FROM users WHERE uid=?",[friendList[key]],function(err,results,fields){
        list[name]=results[0].name;
        list[email]=results[0].email;
        list[id]=results[0].uid;
      });

    }
  }
};
exports.printJSON=function(response)
{
  var num=response.length;
  var i=0;
  if(num==0)
  console.log("there are no objects in json\n");
  console.log("printing out elements of json for json:"+response+"-\n");
